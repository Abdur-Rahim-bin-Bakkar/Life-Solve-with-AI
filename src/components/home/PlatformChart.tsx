...
import { BarChart3, MessageCircle, CheckCircle, FileText } from "lucide-react"
...
export default function PlatformChart() {
  ...
  const maxVal = Math.max(...stats.daily.map((d) => Math.max(d.posts, d.solved, d.comments)), 1)
                ...
                return (
                  <div key={d.date} className="flex flex-1 flex-col items-center gap-1.5">
                    <div className="relative flex w-full items-end justify-center gap-0.5 rounded-lg" style={{ height: 140 }}>
                      <div className="w-[30%] rounded-t-sm bg-teal-400 transition-all duration-500" style={{ height: `${pH}%` }} title={`${d.posts} posts`} />
                      <div className="w-[30%] rounded-t-sm bg-emerald-500 transition-all duration-500" style={{ height: `${sH}%` }} title={`${d.solved} solved`} />
                      <div className="w-[30%] rounded-t-sm bg-violet-400 transition-all duration-500" style={{ height: `${cH}%` }} title={`${d.comments} comments`} />
                    </div>
